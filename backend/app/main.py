...
from backend.app.threads import router as threads_router
from backend.app.ai_wallet import router as ai_wallet_router
...
from backend.app import email_service as email_module  # noqa: F401  (declares tables)
...
from backend.app.email_service import router as email_router
...
app.include_router(ai_wallet_router)
...
class WebhookEvent(BaseModel):
    """Minimal Stripe webhook event structure for type checking."""
    type: str
    data: dict


class ProvisionTenantRequest(BaseModel):
    """Request to provision a new tenant (Business + Location + initial Manager)."""
    business_name: str
    location_name: str
...
async def authentication_middleware(request: Request, call_next):
    ...
    try:
        if requested_business:
            business_id = int(requested_business)
        elif memberships:
            business_id = memberships[0].business_id
        else:
            # User has no memberships — should not happen after proper onboarding
            from fastapi.responses import JSONResponse
            return JSONResponse(status_code=403, content={"detail": "You are not assigned to any business. Contact an administrator."})
    except ValueError:
        from fastapi.responses import JSONResponse
        return JSONResponse(status_code=400, content={"detail": "Invalid business workspace"})

    # Validate that the user has access to the requested/chosen business
    if not any(x.business_id == business_id for x in memberships):
        from fastapi.responses import JSONResponse
...
def get_my_memberships(request: Request):
    """Get all businesses/workspaces the user belongs to. Used for workspace switcher."""
    ...
    with Session(engine) as session:
        ...
        return [
            {
                "business_id": m.Membership.business_id,
...
def get_subscription_status(request: Request):
    """
    Get the current subscription status for the business.

    Returns subscription details or null if not subscribed.
    """
...
async def stripe_webhook(request: Request):
    """
    Stripe webhook endpoint. Listens for subscription lifecycle events.

    This endpoint should be added to Stripe's webhook settings with the webhook secret.
    """
...
@app.post("/admin/provision-tenant")
def provision_tenant(payload: ProvisionTenantRequest, request: Request):
    """
    Provision a new tenant (Business + Location + Manager account).

    Requires ADMIN_API_KEY in Authorization header: "Bearer <key>"

    This endpoint creates:
    - A new Business with the given name
    - A new Location within that business
    - A new Manager UserAccount
    - A Membership linking the manager to the business with "owner" role
    - Default ManagerSettings and BusinessModule configuration
    """
    ...
    with Session(engine) as session:
        ...
        # Create Manager UserAccount
        if username_exists(payload.manager_username):
        ...
        # Create ManagerSettings with sensible defaults for a restaurant
        manager_settings = ManagerSettings(
            business_id=business.id,
            store_name=payload.store_name or payload.location_name,
        ...
        # Enable Scheduler module for this business
