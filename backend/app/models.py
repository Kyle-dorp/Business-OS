...
class Subscription(SQLModel, table=True):
    """Stripe subscription tied to a Business. Drives feature access and module enablement."""
    id: Optional[int] = Field(default=None, primary_key=True)
    business_id: int = Field(index=True)
    stripe_customer_id: str = Field(index=True)
    stripe_subscription_id: str = Field(index=True)
    status: str  # active, past_due, trialing, canceled, incomplete, incomplete_expired
    plan: str = ""  # e.g., "starter", "professional", "enterprise"
    current_period_end: str  # ISO format timestamp
    created_at: str = Field(default_factory=utc_now_iso)
    updated_at: str = Field(default_factory=utc_now_iso)
...
class Budget(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    business_id: int = Field(index=True)
    period: str = Field(index=True)          # YYYY-MM-DD (first of month)
    account_id: int = Field(index=True)
    budget_cents: int = 0
    notes: str = ""
    created_at: str = Field(default_factory=utc_now_iso)
