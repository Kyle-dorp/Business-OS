        ...
        <section className="card fin-section">
          ...
          <div className="fin-mini-table">
            ...
            <div className="fin-mini-row highlight">
              ...
              <span>{money((sum.receivables_cents || 0) - (sum.payables_cents || 0))}</span>
            </div>
          </div>
        </section>

        <section className="card fin-section">
          <h2 className="fin-section-title">Profit &amp; loss snapshot</h2>
          <div className="fin-mini-table">
...
function BudgetTab() {
  ...
  async function load() {
    const [b, a, pl] = await Promise.all([
      api("/platform/finance/budgets"),
  ...
  async function save() {
    try {
      setMsg(""); setOk(false);
      await api("/platform/finance/budgets", {
        method: "POST",
        body: JSON.stringify({
      ...
      <section className="card fin-section" style={{ marginBottom: 20 }}>
        ...
        <div className="os-form-grid">
          ...
          <Field label="Notes (optional)">
            <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="e.g. Q2 target" />
          </Field>
        </div>
        ...
        return (
          <section key={period} className="card fin-section" style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
              <h2 className="fin-section-title" style={{ margin: 0 }}>
                {new Date(period + "-15").toLocaleDateString("en-US", { month: "long", year: "numeric" })}
              </h2>
              <span style={{
...
function CashFlowTab() {
  ...
  async function load() {
    try {
      setMsg("");
      setData(await api(`/platform/finance/cashflow?start=${form.start}&end=${form.end}`));
    } catch (e) { setMsg(e.message); }
  }
  useEffect(() => { load(); }, []);
      ...
      {data && (
        <>
          ...
          <section className="card fin-section" style={{ marginBottom: 18 }}>
            ...
            <Table headers={["Date", "Source", "Description", "Amount"]}
              rows={(data.inflow_rows || []).map((r) => [r.date, r.source, r.description, money(r.amount_cents)])}
              footer={["", "", "Total inflows", money(data.inflows_cents)]} />
          </section>
          <section className="card fin-section">
            <h2 className="fin-section-title">Cash paid out</h2>
            <Table headers={["Date", "Source", "Description", "Amount"]}
              rows={(data.outflow_rows || []).map((r) => [r.date, r.source, r.description, money(r.amount_cents)])}
              footer={["", "", "Total outflows", money(data.outflows_cents)]} />
          </section>
        </>
      )}
    </div>
...
function AccountsTab() {
  ...
  const SUBTYPES = {
    asset: ["cash", "accounts_receivable", "inventory", "fixed_asset", "other"],
    liability: ["accounts_payable", "credit_card", "loan", "other"],
    equity: ["owner_equity", "retained_earnings", "other"],
    income: ["sales", "services", "other"],
    expense: ["cost_of_goods_sold", "payroll", "rent", "utilities", "supplies", "other"],
  };
  ...
  async function save() {
    try {
      ...
      setOk(true); setMsg("Account added.");
      setForm({ code: "", name: "", account_type: "expense", subtype: "" });
      await load();
    } catch (e) { setMsg(e.message); }
  }
  ...
  const grouped = useMemo(() => {
    ...
    accounts.forEach((a) => { if (map[a.account_type]) map[a.account_type].push(a); });
    return map;
  }, [accounts]);
    ...
    <div>
      ...
      <section className="card fin-section" style={{ marginBottom: 20 }}>
        ...
        <div className="os-form-grid">
          ...
          <Field label="Type">
            <select value={form.account_type} onChange={(e) => setForm({ ...form, account_type: e.target.value, subtype: "" })}>
...
function PayrollTab() {
  ...
  const [form, setForm] = useState({
    ...
    gross_wages: "", employer_taxes: "", deductions: "",
  ...
  async function save() {
    try {
      setMsg(""); setOk(false);
      await api("/platform/finance/payroll", {
        method: "POST",
        body: JSON.stringify({
          ...form,
          gross_wages: Number(form.gross_wages),
          employer_taxes: Number(form.employer_taxes || 0),
  ...
  return (
    <div>
      ...
      <section className="card fin-section" style={{ marginBottom: 20 }}>
        ...
        <Field label="Notes">
          <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Optional notes or reference" style={{ maxWidth: 500 }} />
        </Field>
        <br />
      ...
      <section className="card fin-section">
        ...
        <Table
          headers={["Period", "Gross wages", "Employer taxes", "Deductions", "Net pay", "Paid from", "Notes"]}
          rows={runs.map((r) => [
            `${r.period_start} → ${r.period_end}`,
            money(r.gross_wages_cents), money(r.employer_taxes_cents), money(r.deductions_cents),
            <strong>{money(r.net_pay_cents)}</strong>,
            r.payment_account_name, r.notes || "—",
          ])}
          footer={runs.length ? ["Total", money(totalGross), money(totalTax), "", money(totalNet), "", ""] : null}
        />
      </section>
    </div>
  );
