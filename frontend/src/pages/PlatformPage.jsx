...
export default function PlatformPage({ section = "overview" }) {
  ...
  if (section === "tasks") return <div className="page os-page"><Header title="Tasks & checklists" copy="Keep operational work beside the financial picture." /><section className="card os-form"><div className="os-form-grid"><Field label="Task"><input value={form.title || ""} onChange={(e) => setForm({ ...form, title: e.target.value })} /></Field><Field label="Due"><input type="date" value={form.due_date || ""} onChange={(e) => setForm({ ...form, due_date: e.target.value })} /></Field><Field label="Priority"><select value={form.priority || "normal"} onChange={(e) => setForm({ ...form, priority: e.target.value })}><option>low</option><option>normal</option><option>high</option><option>urgent</option></select></Field></div><button className="primary-btn" onClick={() => submit("/platform/tasks", form)}>Add task</button></section><Notice text={message} /><Table headers={["Task", "Due", "Priority", "Status"]} rows={(data || []).map((x) => [x.title, x.due_date || "—", x.priority, x.status])} /></div>;
