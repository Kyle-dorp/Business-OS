...
export default function SettingsPage({ user, workspaceRole, modules = [], onModulesChanged, onUserChange, onLogout }) {
  ...
  return <div className="page">
    ...
    {user.role === "manager" && ["owner", "admin"].includes(workspaceRole) && <section className="card module-settings-card">
      ...
      <div className="module-toggle-grid">{MODULE_OPTIONS.map(([key, label, description]) => <label className="module-toggle-row" key={key}><div><strong>{label}</strong><span>{description}</span></div><input type="checkbox" checked={moduleState[key] !== false} disabled={modulesBusy} onChange={(event) => setModule(key, event.target.checked)} /></label>)}</div>
      <p className="module-safety-note">Overview and Settings always remain visible.</p>
    </section>}
